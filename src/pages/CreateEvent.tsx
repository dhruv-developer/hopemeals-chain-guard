...
import { useNavigate } from "react-router-dom";
import { Plus, MapPin, Users, Package, AlertCircle, Shield } from "lucide-react";
import { Layout } from "@/components/layout/layout";
import { PageHeader } from "@/components/ui/page-header";
import { ForensicCard } from "@/components/ui/forensic-card";
import { Button } from "@/components/ui/button";
...
export default function CreateEvent() {
  ...
  useEffect(() => {
    api<BCStatus>("/blockchain/status")
      .then(setBcStatus)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        quantity: parseInt(formData.quantity),
        unit: formData.unit,
        gps: { lat: parseFloat(formData.gps_lat), lon: parseFloat(formData.gps_lon) },
        timestamp: formData.timestamp || new Date().toISOString(),
        device_id: formData.device_id,
        ip: formData.ip,
        beneficiary_ids: formData.beneficiary_ids
          .split(",")
          .map((id) => id.trim())
          .filter(Boolean),
      };
      ...
      setSuccess(`Event created successfully! ID: ${response.event_id}`);
      toast({ title: "Event created", description: `Event ${response.event_id} has been recorded` });
      localStorage.setItem("lastEventId", response.event_id);

      setTimeout(() => navigate(`/events/${response.event_id}`), 1200);
    } catch (e: any) {
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">
              Anchoring: {bcStatus?.mode ? bcStatus.mode.toUpperCase() : "—"}
            </Badge>
          </div>
        }
      />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("timestamp", e.target.value)}
              />
              <p className="text-sm text-muted-foreground">Leave empty to use current time</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={3}
              />
              <p className="text-sm text-muted-foreground">Comma-separated list of beneficiary identifiers</p>
            </div>
