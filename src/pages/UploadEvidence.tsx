...
// UploadEvidence.tsx
import { useState, useEffect } from "react";
import { Upload, File, Image, FileText, Database, Eye, AlertCircle, Shield, Hash } from "lucide-react";
import { Layout } from "@/components/layout/layout";
import { PageHeader } from "@/components/ui/page-header";
import { ForensicCard } from "@/components/ui/forensic-card";
import { Button } from "@/components/ui/button";
...
export default function UploadEvidence() {
  ...
  useEffect(() => {
    const lastEventId = localStorage.getItem("lastEventId");
    if (lastEventId) setEventId(lastEventId);
    api<BCStatus>("/blockchain/status").then(setBcStatus).catch(() => setBcStatus(null));
  }, []);
  ...
  const runELA = async () => {
    ...
    try {
      ...
      setResult((prev: any) => ({ ...prev, ela: elaResult }));
      toast({
        title: "ELA analysis complete",
        description: `Suspicion level: ${elaResult.suspicion?.toFixed(2) || "Unknown"}`,
      });
    } catch (e: any) {
      toast({ title: "ELA analysis failed", description: e.message, variant: "destructive" });
    }
  };

  // Optional: manually anchor the file's sha256 (demo only)
  const anchorFileHash = async () => {
    if (!result?.sha256) return;
    try {
      const out = await api("/blockchain/anchor", {
        method: "POST",
        body: { record_hash: result.sha256 },
      });
      toast({
        title: "Manual anchor submitted",
        description: out?.mode === "eth" ? `tx: ${out.tx_hash?.slice(0, 12)}...` : `mode: ${out?.mode || "off"}`,
      });
    } catch (e: any) {
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">Anchoring: {bcStatus?.mode ? bcStatus.mode.toUpperCase() : "—"}</Badge>
          </div>
        }
      />

      <div className="max-w-2xl mx-auto space-y-8">
        <ForensicCard title="Evidence Submission" description="All uploads are hashed and chained in the ledger">
          <form onSubmit={handleSubmit} className="space-y-6">
            {error && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
            ...
            <Button type="submit" className="w-full bg-gradient-to-r from-primary to-primary-hover" disabled={loading}>
              <Upload className="h-4 w-4 mr-2" />
              {loading ? "Processing..." : "Upload Evidence"}
            </Button>
          </form>
        ...
        {result && (
          <ForensicCard title="Upload Results" description="Evidence successfully processed and stored">
            <div className="space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium">Evidence ID</Label>
              ...
              <div className="flex items-center gap-2">
                <Label className="text-sm font-medium">Ledger Index</Label>
                <Badge variant="outline">{result.ledger_index}</Badge>
              </div>

              {/* Anchoring status and optional manual anchor */}
              <div className="rounded-lg border p-3 bg-accent/30">
                <div className="flex items-center gap-2 mb-2">
                  <Shield className="h-4 w-4 text-primary" />
                  <span className="text-sm">
                    Anchoring mode: <strong>{bcStatus?.mode ? bcStatus.mode.toUpperCase() : "—"}</strong>
                  </span>
                </div>
                <p className="text-xs text-muted-foreground">
                  When anchoring is enabled, the system automatically anchors the <em>ledger record hash</em>. You can optionally anchor
                  this file&apos;s SHA-256 below for demo purposes.
                </p>
                <div className="mt-2">
                  <Button variant="outline" size="sm" onClick={anchorFileHash}>
                    <Hash className="h-4 w-4 mr-2" />
                    Anchor file hash (optional)
                  </Button>
                </div>
              </div>
              ...
              {result.ela && (
                <div className="border border-border rounded-lg p-4 bg-accent/50">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      High suspicion suggests potential tampering but is not conclusive. Corroborate with other evidence.
                    </p>
                  </div>
                </div>
              )}
